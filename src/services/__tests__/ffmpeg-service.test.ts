...
describe('FFmpegService', () => {
  ...
  describe('generateCaptionFile', () => {
    it('should generate synchronized SRT subtitle file using subtitle library', async () => {
      mockFs.writeFile.mockResolvedValue(undefined)
      ...
      // Should be valid SRT format generated by subtitle library
      expect(writtenContent).toMatch(/^\d+\n\d{2}:\d{2}:\d{2},\d{3} --> \d{2}:\d{2}:\d{2},\d{3}\n.+/)
      
      // Should have multiple SRT entries (script is broken into 2-word chunks)
      ...
      // Check formatting - should have correct number of entries
    ...
    it('should handle short scripts properly using subtitle library', async () => {
      mockFs.writeFile.mockResolvedValue(undefined)
      ...
      expect(writtenContent.split('\n').filter(line => /^\d+$/.test(line))).toHaveLength(2) // Should have 2 entries
    })
  })
